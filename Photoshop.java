...
public class Photoshop extends Application {
    ...
    public void start(Stage stage) throws FileNotFoundException {
        ...
        Image image = new Image(new FileInputStream("yobama.jpg"));
    ...
    public Image crossCorrelation (Image image) {
        ...
        for(int y = offset; y < height-offset; y++) {
            for(int x = offset; x < width-offset; x++) {
                sum[0][y][x] = (int) (((sum[0][y][x]-minVal)*255)/(maxVal-minVal));
                sum[1][y][x] = (int) (((sum[1][y][x]-minVal)*255)/(maxVal-minVal));
                sum[2][y][x] = (int) (((sum[2][y][x]-minVal)*255)/(maxVal-minVal));
